export default function Summary({ results, config, onRestart, onRetry }) {
  ...
  const msg =
    pct >= 90 ? "You're ready for the exam. Outstanding performance!" :
    pct >= 75 ? "Strong performance — review the questions you missed and you'll be set." :
    pct >= 60 ? "Good progress. Focus on the topics below and try again." :
            ...
            <div key={cat} className="review-cat">
              ...
              {items.map((r, i) => (
                <div key={i} className="review-item">
                  <p className="review-q">{r.question.q}</p>
                  <p className="review-answer">
                    <span className="review-label">Correct answer: </span>
                    {r.question.opts[r.question.answer]}
                  </p>
                  <p className="review-explanation">{r.question.fb_wrong}</p>
                </div>
              ))}
            </div>
          ))}
